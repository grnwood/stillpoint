...
def load_seed_agents_workspace(default: bool = True) -> bool:
    """Return whether terminal startup should seed agent workspace files."""
    payload = _read_global_config()
...
def save_seed_agents_workspace(enabled: bool) -> None:
    """Persist whether terminal startup should seed agent workspace files."""
    _update_global_config({"seed_agents_workspace": bool(enabled)})


def load_terminal_settings() -> dict:
    """Return global embedded-terminal preferences with safe defaults."""
    payload = _read_global_config()
    raw = payload.get("terminal_settings")
    raw = raw if isinstance(raw, dict) else {}
    arguments = raw.get("shell_arguments")
    if not isinstance(arguments, list):
        arguments = []
    try:
        scrollback = int(raw.get("scrollback", 10_000))
    except (TypeError, ValueError):
        scrollback = 10_000
    try:
        ...
        font_size = 0
    return {
        "shell_executable": str(raw.get("shell_executable") or "").strip(),
        "shell_arguments": [str(item) for item in arguments],
        "scrollback": max(100, min(200_000, scrollback)),
        "font_family": str(raw.get("font_family") or "").strip(),
        "font_size": 0 if font_size <= 0 else max(6, min(72, font_size)),
    }


def save_terminal_settings(settings: dict) -> None:
    """Persist global terminal preferences without storing live session state."""
    settings = settings if isinstance(settings, dict) else {}
    arguments = settings.get("shell_arguments")
    if not isinstance(arguments, list):
        arguments = []
    try:
        scrollback = int(settings.get("scrollback", 10_000))
    except (TypeError, ValueError):
        scrollback = 10_000
    try:
        font_size = int(settings.get("font_size", 0))
    except (TypeError, ValueError):
        font_size = 0
...
def _rebase_page_path(page_path: str, old_folder: str, new_folder: str) -> str:
    ...
    old_path = Path(cleaned_old)
    try:
        relative = page_folder.relative_to(old_path)
    except ValueError:
        old_parts = old_path.parts
        page_parts = page_folder.parts
        if len(old_parts) > len(page_parts):
            raise
        for idx, part in enumerate(old_parts):
            if page_parts[idx].lower() != part.lower():
                raise
        relative = Path(*page_parts[len(old_parts):])
...
def validate_move_tree_index(old_folder_path: str, new_folder_path: str) -> None:
    """Validate that a tree move would not create conflicts. Raises RuntimeError if invalid."""
    conn = _connect_to_vault_db()
    try:
        ...
        rows = conn.execute(
            "SELECT path FROM pages WHERE deleted = 0 AND (path = ? OR path LIKE ?)",
            (old_page_path, like_pattern),
        ).fetchall()
        if not rows:
            return  # Nothing to move, no conflicts
        
        # Build the path map to check for duplicates
        ...
        # Check for conflicts with existing pages (that aren't being moved)
        placeholders = ",".join("?" for _ in new_paths)
        if placeholders:
            existing = {
                row[0]
                for row in conn.execute(
                    f"SELECT path FROM pages WHERE deleted = 0 AND path IN ({placeholders})",
                    new_paths,
                ).fetchall()
            }
...
def move_tree_index(old_folder_path: str, new_folder_path: str, root: Path, *, set_new_parent_order: bool = False) -> dict[str, dict]:
    ...
    try:
        ...
        like_pattern = f"{old_prefix}/%"
        rows = conn.execute(
            "SELECT path, display_order FROM pages WHERE deleted = 0 AND (path = ? OR path LIKE ?)",
            (old_page_path, like_pattern),
        ).fetchall()
        if not rows:
