...
- In the page picker, vi mode uses `Ctrl+Shift+J/K` (on macOS: `Control+J` / `Control+K`) to navigate suggestions and `Enter` to select one. Suggestions are never selected automatically, and `Esc` clears the search.
