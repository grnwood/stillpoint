...
async def get_current_user(
    ...
    try:
        ...
        if payload.get("scope") or payload.get("type"):
            raise HTTPException(status_code=401, detail="Invalid access token scope")
...
def auth_mcp_token(
    ...
    """Issue a revocable terminal-session token scoped to the active vault."""
    ...
    claims = {
        ...
        "sid": str(payload.session_id or "")[:128],
    }
...
def _mcp_user(claims: dict) -> AuthModels.UserInfo:
    ...
    return AuthModels.UserInfo(
        username=str(claims.get("sub") or "terminal"),
        is_admin=False,
        can_write=can_write,
        role="normal",
...
async def mcp_endpoint(
    ...
    """Minimal client-neutral MCP Streamable HTTP endpoint for terminal sessions."""
    session_id = str(request.headers.get(_REMOTE_CONTEXT_HEADER) or "").strip()
