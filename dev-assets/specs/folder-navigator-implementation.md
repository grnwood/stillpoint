...
- Root boundary validation before internal file opening, no API/client calls, and a filename-only per-root SQLite cache in `.sp_folder`; file contents are never indexed.
...
4. **Search and Quick Open:** Search results are grouped with file headers in a list rather than a hierarchical results view. Quick Open uses a persistent `.sp_folder/catalog.sqlite3` filename cache and refreshes it lazily in the background; nested changes outside watched/expanded directories may appear during the next Quick Open refresh. Quick Open batches `.gitignore` checks while cataloging, but content search invokes Git per path and needs batching for large repositories. Quick Open's modal UI reports whether the cache is indexing but does not yet show precise progress.
